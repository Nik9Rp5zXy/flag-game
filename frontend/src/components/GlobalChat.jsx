                   ...
                   return (
                     <div key={msg.id} className={`flex flex-col group relative ${isMe ? 'items-end' : 'items-start'} ${isSystem ? 'items-center my-2' : ''}`}>
                       ...
                       <div className="relative group/msg">
                          ...
                          {/* Message Bubble */}
                          <div className={`relative px-3 py-2 rounded-xl text-sm max-w-[240px] break-words border ${rs.border} ${rs.bg} ${rs.text} ${rs.glow}`}>
                            {msg.text}
                          ...
                          {/* Hover Actions (Reply, Emoji, Delete) */}
                          {!isSystem && profile?.isAuthenticated && (
                            <div className={`absolute top-1/2 -translate-y-1/2 flex items-center gap-1 opacity-0 group-hover/msg:opacity-100 transition-opacity bg-black/80 p-1 rounded-lg border border-gray-700 shadow-xl z-10 ${isMe ? 'right-full mr-2' : 'left-full ml-2'}`}>
                               <button onClick={() => setReplyTo(msg)} className="p-1 hover:text-neon-blue text-gray-400" title="Yanıtla"><Reply className="w-4 h-4" /></button>
                               <button onClick={() => setActiveReactionMsg(msg.id === activeReactionMsg ? null : msg.id)} className="p-1 hover:text-yellow-400 text-gray-400 relative" title="Emoji">
                                 <Smile className="w-4 h-4" />
                               </button>
                               {(profile.role === 'owner' || profile.role === 'admin') && (
                                 <button onClick={() => handleDelete(msg.id)} className="p-1 hover:text-red-500 text-gray-400" title="Sil"><Trash2 className="w-4 h-4" /></button>
                               )}

                               {/* Emoji Picker Popup */}
                               {activeReactionMsg === msg.id && (
                                 <div className="absolute bottom-full mb-2 bg-gray-800 border border-gray-600 rounded-lg p-1 flex gap-1 shadow-2xl">
                                    {EMOJIS.map(e => (
                                      <button key={e} onClick={() => handleReact(msg.id, e)} className="hover:scale-125 transition-transform text-lg px-1">{e}</button>
                                    ))}
                                 </div>
                               )}
                            </div>
